...
const DarkModeToggle = () => {
  ...
  return (
    <button
      ...
      className="fixed bottom-5 right-4 z-[110] flex items-center gap-3 rounded-full border border-slate-200/80 bg-white/90 px-3 py-3 shadow-[0_18px_40px_-18px_rgba(15,23,42,0.6)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:shadow-[0_22px_50px_-18px_rgba(15,23,42,0.7)] dark:border-slate-700/80 dark:bg-slate-900/90 sm:bottom-6 sm:right-6"
    >
      <span className="relative flex h-8 w-[72px] items-center rounded-full bg-slate-200/90 p-1 dark:bg-slate-800">
        <span
          className={`absolute top-1 h-6 w-6 rounded-full bg-white shadow-md transition-all duration-300 dark:bg-slate-950 ${isDark ? "left-[42px]" : "left-1"
            }`}
        />

        <span className="relative z-10 flex w-full items-center justify-between px-1">
          <span
            className={`flex h-4 w-4 items-center justify-center transition ${isDark ? "text-slate-500" : "text-amber-500"
              }`}
          >
            <svg
              viewBox="0 0 24 24"
          ...
          <span
            className={`flex h-4 w-4 items-center justify-center transition ${isDark ? "text-sky-200" : "text-slate-500"
              }`}
          >
